...
**장애가 발생할 수 있는 환경에서의 대처 자세**
- 장애가 아예 발생하지 않는 시스템은 없다가는 사실을 인정하기
- 장애 대응을 위해 고민해야 할 요소
- 체계화된 장애 대응 프로세스 구축하기
  - 장애 발생시엔 침착함을 유지하고 해결방법을 즉시 생각하기 쉽지 않다.  따라서 장애가 발생했을 때 기계적으로 대응할 수 있는 메뉴얼을 만들어놓는 것이 좋다.

**어떻게 장애 대응 프로세스를 만들 수 있을까?**
- 장애가 발생할 수 있는 포인트들에 대해 고민
- 장애를 대응하고 개선할 방법이 무엇일지에 대해 고민
- 로깅/Alert 파이프라인을 통한 장애 예방/탐지

**현업에서는 어떤 지표를 주로 볼까?**
- 인스턴스의 리소스 지표: CPU, Memory, Latency, Disk
- 데이터를 파싱하는데 실패한 경우 이에 대한 로그
- 에러의 빈도가 잦은 경우에 대한 알림
- Request/Response 로그: 서비스간의 상호작용을 파악하는데 용이
...
**부하테스트 시 확인해야 하는 요소**
- 예상 TPS(Transaction Per Second)
- 평균/중간/최대 응답시간
  - Percentile
- 다량의 트래픽 유입 시 동시성 이슈 발생 여부

**고가용성(High Availability)
- 서비스가 오랜 기간 동안 지속적으로 정상 운영 가능하도록 하는 성질
- SPOF 지점을 찾아서 이에 대한 해결 방안을 고민 해야함
...
- 일반적인 해결 방법
  ...
  - 스로틀링 / 서킷 브레이커
- 추가적인 고려 요소
  - 특정 API가 오래 걸리거나 계속 실패해 서버 리소스가 부족한 경우
  - DB 리소스 부족 문제로 인해 DB 서버에 연결이 불가능한 경우
...
**장애를 발생 시키는 대표적인 요소(Application 레벨 기준)**
- 장애란 특정 상황이 발생해 현재 운영 중인 서버가 정상 작동이 불가능한 상태를 의미
  - Lack of Resource    : 리소스 부족 
  - Unhandled Exception : 예외 처리를 놓쳤을 경우(예: 파싱 에러)
  - Slow Query, I/O     : 응답 지연

- 유저가 우리 서비스를 어떻게 사용할지를 정확히 예측해 내는 것은 어렵다.
...
**장애 보고서의 필수 구성**
- 장애 현상 기술: 타임라인, 영향 범위, 비즈니스 임팩트
- 장애 조치 내용: 장애 원인, 해소 타임라인, 대응 방법, 후속 대응 계획
- 장애 상세 분석: 5-Why 분석
- 액션 아이템: 단기적 방안, 중장기적 방안, 장기적 방안
