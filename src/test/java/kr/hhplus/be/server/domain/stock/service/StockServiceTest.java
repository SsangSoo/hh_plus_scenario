...
class StockServiceTest extends SpringBootTestSupport {
    ...
    void deductedStockHappyCase() throws InterruptedException {
        ...
        ProductResponse productResponse = productService.registerProduct(new RegisterProductServiceRequest("고급 볼펜", 1000L));

        stockService.addStock(new AddStock(productResponse.getId(), 30000L));

        // when
        for(int i = 0; i < threadCount; i++) {
            es.submit(() -> {
                try {
                    stockService.deductedStock(productResponse.getId(), 3L);
                } finally {
                    latch.countDown();
                }
            });
        ...
        // then
        Stock findStock = stockRepository.findByProductIdAndDeletedFalse(productResponse.getId()).orElseThrow();
    ...
    void deductedStock() throws InterruptedException {
        ...
        stockService.addStock(new AddStock(productResponse.getId(), 30L));
        ...
        for(int i = 0; i < threadCount; i++) {
            es.submit(() -> {
                try {
                    stockService.deductedStock(productResponse.getId(), 4L);
        ...
        // then
        Stock findStock = stockRepository.findByProductIdAndDeletedFalse(productResponse.getId()).orElseThrow();
        Assertions.assertThat(findStock.getQuantity()).isEqualTo(2L);
